...
import { Mail, Lock, LogIn, Loader2, AlertCircle, Eye, EyeOff, ShieldCheck } from 'lucide-react';
...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setApiError(resp?.message || 'Invalid email or password. Please try again.');
    } finally {
  ...
  return (
    <div className="w-full max-w-md bg-white rounded-2xl p-6 sm:p-8 shadow-xl shadow-slate-200/60 border border-slate-200/80 text-slate-900">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-xs text-slate-500 mt-1">Access your smartphone orders, wishlist, and dashboard</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        <button
          type="submit"
          disabled={isSubmitting}
          className="w-full flex items-center justify-center gap-2 py-3 px-4 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 active:bg-blue-800 rounded-xl shadow-md hover:shadow-lg transition duration-200 disabled:opacity-60 cursor-pointer"
        >
          {isSubmitting ? (
            <>
